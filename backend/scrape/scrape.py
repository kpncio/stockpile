...
def calc_dixie(e, y, p, d, k, f):
    return round(50.14348112 * (e ** -0.576) * (y ** 0.136) * (p ** -0.119) * (d ** 0.091) * (k ** 0.042) * (f ** 0.036), 3)
...
def get_index_prices():
    ...
    usdx_change = usdx_price - calc_dixie(usdx['eurusd'][-2], usdx['usdjpy'][-2], usdx['gbpusd'][-2], usdx['usdcad'][-2], usdx['usdsek'][-2], usdx['usdchf'][-2])
    usdx_percent = usdx_change / usdx_price * 100
    ...
    cursor.executemany('INSERT INTO `index` (`symbol`, `value`, `change`, `percent`) VALUES (%s, %s, %s, %s)', [
        ('SPX', indices[0][1], indices[0][2], indices[0][3]),
        ('NDX', indices[1][1], indices[1][2], indices[1][3]),
        ('DJIA', indices[2][1], indices[2][2], indices[2][3]),
        ('FOREX', usdx_price, usdx_change, usdx_percent),
        ('CRYPTO', btc_price, btc_change, btc_percent),
        ('METALS', gold_price, gold_change, gold_percent),
        ('ENERGY', oil_price, oil_change, oil_percent),
        ('PORTFOLIO', portfolio_price, portfolio_change, portfolio_percent)
    ])
...
def get_yahoo_intra_prices():
    ...
    url = f"https://api.cloudflare.com/client/v4/accounts/{os.getenv('cf_account')}/storage/kv/namespaces/{os.getenv('cf_kv_intra')}/bulk"
    db = mysql.connector.connect(
        host=os.getenv('db_host'),
        user=os.getenv('db_user'),
        password=os.getenv('db_pass'),
    ...
    for symbol in symbols:
        ...
        for index, rows in data.iterrows():
            ...
            # For JSON use '%Y-%m-%dT%H:%M:%S.%fZ'
            index = '-'.join(str(index).split('-')[:-1])
            epoch = timegm(time.strptime(index, '%Y-%m-%d %H:%M:%S'))
...
def get_yahoo_daily_prices():
    ...
    url_preview = f"https://api.cloudflare.com/client/v4/accounts/{os.getenv('cf_account')}/storage/kv/namespaces/{os.getenv('cf_kv_preview')}/bulk"
    url_daily = f"https://api.cloudflare.com/client/v4/accounts/{os.getenv('cf_account')}/storage/kv/namespaces/{os.getenv('cf_kv_daily')}/bulk"
    db = mysql.connector.connect(
        host=os.getenv('db_host'),
        user=os.getenv('db_user'),
        password=os.getenv('db_pass'),
...
def get_investing_prices():
    ...
    for row in table.tbody.find_all('tr'):
        ...
        if(columns != []):
            ...
            prices[f'{date[2]}-{month[date[0]]}-{date[1]}'] = {
...
def get_metadata_fill():
    ...
    log('Priming mode actived: Retrieving all required metadata (over 500 calls)...')
    ...
    def metadata(chunk):
        ...
        for symbol in chunk:
            ...
            value = {'name': data['name'], 'description': data['description'], 'exchange': data['exchangeCode'], 'joined': data['startDate'], 'access': True}
            with lock:
                values_cf.append({'key': symbol, 'value': json.dumps(value)})
                values_db.append((symbol, data['name'], data['description'], data['exchangeCode'], 'STOCK', data['startDate'], True))
    ...
    values_rt['FOREX'] = strip(predefined)
    values_sr = strip(predefined, values_sr)

    completed = 0
    print(f'   ({completed}/{len(predefined)})', end='\r')
    for each in predefined:
        value = {'name': each[1], 'description': each[3], 'exchange': each[2], 'joined': '2000-01-01', 'access': True}
        values_cf.append({'key': each[0], 'value': json.dumps(value)})
        values_db.append((each[0], each[1], each[3], each[2], 'FOREX', '2000-01-01', True))
    ...
    values_rt['CRYPTO'] = strip(predefined)
    values_sr = strip(predefined, values_sr)

    completed = 0
    print(f'   ({completed}/{len(predefined)})', end='\r')
    for each in predefined:
        value = {'name': each[1], 'description': each[2], 'exchange': 'Tiingo', 'joined': '2000-01-01', 'access': True}
        values_cf.append({'key': each[0], 'value': json.dumps(value)})
        values_db.append((each[0], each[1], each[2], 'Tiingo', 'CRYPTO', '2000-01-01', True))
    ...
    values_rt['METALS'] = strip(predefined)
    values_sr = strip(predefined, values_sr)

    completed = 0
    print(f'   ({completed}/{len(predefined)})', end='\r')
    for each in predefined:
        value = {'name': each[1], 'description': each[4], 'exchange': each[2], 'joined': '2000-01-01', 'access': each[3]}
        values_cf.append({'key': each[0], 'value': json.dumps(value)})
        values_db.append((each[0], each[1], each[4], each[2], 'METALS', '2000-01-01', each[3]))
    ...
    values_rt['ENERGY'] = strip(predefined)
    values_sr = strip(predefined, values_sr)
    ...
    for each in predefined:
        ...
        values_cf.append({'key': each[0], 'value': json.dumps(value)})
        values_db.append((each[0], each[1], each[2], 'NYMEX', 'ENERGY', '2000-01-01', True))
